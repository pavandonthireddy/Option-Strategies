...
data = pd.read_excel('etfs_oct_16_test.xlsx', index_col=None)  
current_date = date(2020,10,10)
expiry_date = date(2020,10,16)
...
Strategies = ["Batman","Bear Double Broken Wing Butterfly","Bull Double Broken Wing Butterfly"]
"""
#######################################################################################
                                    Get Risk Free Date                   
#######################################################################################
...
for i in range(len(All_Option_Chains)):
    ...
    if "Bull Double Broken Wing Butterfly" in Strategies:
        ...
        call_1_pos = list(np.arange(chain.Put_total))
        call_2_pos = list(np.arange(chain.Put_total))
        call_3_pos = list(np.arange(chain.Put_total))
        ...
        for t in itertools.product(*iterables):
            ...
            if pos_1 < pos_2 and pos_2 < pos_3 and pos_cons:
                allocation = np.zeros((chain.Call_total,2))
                allocation[pos_1,1] = call_1_quantity
                allocation[pos_2,1] = -1*call_2_quantity
                allocation[pos_3,1] = call_3_quantity
    ...
    if "Batman" in Strategies:
        Strategy_name = "Batman Spread"
        print("\t Processing ", Strategy_name, " Strategy")
